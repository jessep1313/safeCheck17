...
<head>
    ...
    <title>Auditoria de Inspección | {{ $audit->uuid }}</title>
</head>
...
<body>
    ...
    <table>
        <thead>
            <tr>
                ...
                <th width="45%" rowspan="3" style="text-transform: uppercase; font-size: 1.1rem;">Formato de inspección de compartimentos ocultos
                </th>
    ...
    <table>
        <thead>
            <tr>
                <th class="head-title" colspan="4">AUDITORIA</th>
            </tr>
        </thead>
        <tbody>
            <tr>
                <td style="width: 30%;"><strong>Fecha: </strong>{{ $audit->created_at->format('d/F/Y') }}</td>
                <td colspan="3"><strong>Folio: </strong>{{ $audit->uuid }}</td>
            </tr>
            <tr>
                <td><strong>Hora: </strong>{{ $audit->created_at->format('H:i:s') }}</td>
                <td colspan="3"><strong>Auditor: </strong>{{ $audit->createdBy->name }}</td>
            </tr>
            <tr>
                <td colspan="4" style="color:transparent;">Espacio separador</td>
            </tr>
    ...
    <table>
        <thead>
            <tr>
                <th class="head-title" colspan="4">CHECKLIST</th>
            </tr>
            <tr>
                <th style="width: 30%;">Puntos a revisar</th>
        ...
        <tbody>
            @foreach ($audit->questions as $question)
                <tr>
                    ...
                    <td align="center">{{ $question->answared && $question->result ? "x" : "" }}</td>
                    <td align="center">{{ $question->answared && !$question->result ? "x" : "" }}</td>
    ...
    <table>
        <tbody>
            <tr>
                <td style="border-bottom: none !important;">Comentarios:</td>
            </tr>
            <tr>
                <td style="height: 50px; border-top: none !important;">{{ $audit->comments }}</td>
            </tr>
        </tbody>
    </table>
    ...
    <table align="top">
        <thead>
            <tr>
                <th colspan="2">Inspeccionado por</th>
            </tr>
        </thead>
        <tbody>
            <tr>
                <td style="border-bottom: none !important; width: 50%;">Nombre y firma:</td>
                <td style="border-bottom: none !important; width: 50%;">Firma:</td>
            </tr>
            <tr>
                <td style="height: 20px; border-top: none;">{{ $audit->createdBy->name }}</td>
