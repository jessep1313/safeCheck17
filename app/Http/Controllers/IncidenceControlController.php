...
use App\Exports\IncidenceControlExport;
...
class IncidenceControlController extends Controller
{
    ...
    public function exportPdf(Request $request)
    {
        ...
        $incidences = IncidenceAllView::select(['id', 'uuid', 'type', 'created_at', 'comments', 'evidences'])
            ->orderBy($sortBy, $sort)
            ->searchValues($search)
            ->filterStatus($status)
            ->filterType($type)
            ...
            ->map(function ($row) {
                ...
                return [
                    ...
                    'type' => $row->type,
                    'created_at' => Carbon::parse($row->created_at)->format('d/m/Y, h:i a'),
                    'comments' => $row->comments ? html_entity_decode($row->comments) : 'Sin comentarios',
                    'evidences' => is_array($row->evidences) ? count($row->evidences).' archivo(s)' : 'Sin evidencias',
                    'plan_status' => $plan ? $plan->status->value : 'Sin plan',
                    'plan_finished_at' => $plan && $plan->finished_at ? Carbon::parse($plan->finished_at)->format('d/m/Y, h:i a') : 'N/A',
                ];
            });

        $fileName = 'control-de-incidencias-'.date('ymdhis').'.pdf';
        $pdf = Pdf::loadView('reports.incidences.list', [
    ...
    public function exportExcel(Request $request)
    {
        ...
        $fileName = 'control-de-incidencias-'.date('ymdhis').'.xlsx';

        return Excel::download(new IncidenceControlExport($sort, $sortBy, $search, $status, $type), $fileName);
    }
}
